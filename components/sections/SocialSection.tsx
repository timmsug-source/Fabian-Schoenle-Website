...
type SocialSectionProps = {
  ...
  /**
   * Stellt YouTube und LinkedIn nebeneinander statt untereinander. In den
   * schmalen Spalten passt Text neben Bild nicht mehr — die Blöcke stapeln
...
export default async function SocialSection({ content = {}, nebeneinander = false }: SocialSectionProps) {
  ...
  return (
    <section style={{ background: '#060E1F' }}>
      <div
        className={`max-w-7xl mx-auto px-4 md:px-8 py-20 md:py-28 flex flex-col ${
          nebeneinander ? 'gap-12 md:gap-16' : 'gap-16 md:gap-24'
        }`}
      >
        ...
        <div className={nebeneinander ? 'grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-stretch' : 'contents'}>
        ...
        <div className={nebeneinander ? 'flex flex-col-reverse justify-end gap-8' : 'grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center'}>
          ...
          <div className={`animate-fade-up flex flex-col ${nebeneinander ? 'flex-1' : ''}`}>
            ...
            <a
              ...
              className="cta-metal inline-flex items-center gap-2.5 px-8 py-4 rounded-xl font-inter font-semibold text-sm transition-transform self-center md:self-start mt-auto"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
              </svg>
              <span {...cms('social_youtube_button')}>{txt(content, 'social_youtube_button', 'Jetzt abonnieren')}</span>
            </a>
          </div>
        ...
        <div className={nebeneinander ? 'flex flex-col-reverse justify-end gap-8' : 'grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center'}>
          ...
          <div className={`animate-fade-up flex flex-col ${nebeneinander ? 'flex-1' : ''}`}>
            ...
            >
              ...
              <span {...cms('social_linkedin_button')}>{txt(content, 'social_linkedin_button', 'Jetzt vernetzen')}</span>
            </a>
          </div>
